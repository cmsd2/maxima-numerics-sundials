...
(defun np-cvode-core (rhs-closure neq y0-list t-list lmm rtol atol
  ...
  "CVODE integration core.
   ...
   If events are present, returns [trajectory, events-list]."
...
(defun $np_cvode (&rest args)
  "Integrate ODE system dy/dt = f(t, y) using SUNDIALS CVODE.
   ...
   Expression mode:
     np_cvode([f1,...], [t,y1,...], y0, tspan [, rtol, atol, method, events, max_steps])
   ...
   Returns: 2D ndarray [n_times, 1+neq], or [trajectory, events_list] if events used."
  ...
  (let ((first-arg (first args)))
    (if (numerics-callable-p first-arg)
        ...
        ;; Expression mode: np_cvode(f, vars, y0, tspan [, rtol, atol, method, events, max_steps])
        (progn
